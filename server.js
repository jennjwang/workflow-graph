...
import { retrieveExemplarBlock, retrieveTaskStatements } from './lib/retrieval.js';
...
// O*NET baseline: pure corpus lookup, no LLM task generation. One constrained
// LLM call matches the participant's job to a corpus occupation (inside
// retrieveTaskStatements); we then return that occupation's task statements
// VERBATIM. No interview grounding, no UPPER_LEVEL_TASKS_SYSTEM_PROMPT, no
// rewriting. If nothing matches we return an empty list rather than inventing
// fallback tasks.
app.post('/api/generate-tasks', async (req, res) => {
  const { jobTitle, typicalWeek, responsibilities } = req.body;
  try {
    // k=1000 so we return ALL of the matched occupation's statements, not the
    // default 20.
    const { occupations, statements } = await retrieveTaskStatements(
      { jobTitle, responsibilities, typicalWeek },
      { k: 1000 },
    );
    const tasks = statements.map(name => ({ name }));
    console.log(`[generate-tasks] (onet) role=${jobTitle} matched=${occupations.join('|') || '(none)'} statements=${tasks.length}`);
    res.json({ tasks });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
...
// Streaming variant of /api/generate-tasks — O*NET baseline. No LLM task
// generation: retrieveTaskStatements makes ONE constrained LLM call to match
// the participant's job to a corpus occupation, then we emit that occupation's
// task statements VERBATIM as SSE `task` events, followed by `done`. The SSE
// event contract (`task` / `done` / `error`, `{ name }` payloads) is unchanged
// so the picker frontend keeps working. No interview grounding, no prompt-based
// generation, no rewriting. On no match / empty corpus we emit `done` with zero
// tasks rather than inventing fallbacks.
app.post('/api/generate-tasks-stream', async (req, res) => {
  ...
  try {
    ...
    for (const name of statements) {
      ...
      if (!trimmed) continue;
      sendEvent('task', { name: trimmed });
      emitted += 1;
    }

    console.log(`[generate-tasks-stream] (onet) role=${jobTitle} matched=${occupations.join('|') || '(none)'} emitted=${emitted}`);
