...
// Full SOC list (code + title only) for the screen's "search all occupations"
// fallback. Returns [] on failure (the search box just shows nothing).
export async function listOccupations() {
  try {
    const soc = await loadSoc();
    return soc.map((o) => ({ code: o.code, title: o.title }));
  } catch (err) {
...
export async function occupationCandidates(backgroundTranscript, { excludeCodes = [], hint = '' } = {}) {
  try {
    ...
    const prompt =
      `You are an expert occupation coder. From the FULL O*NET-SOC 2019 list below, pick the TOP ${TOP_N} ` +
      ...
      `Return ONLY JSON: {"ranked":[{"code":"##-####.##","title":"...","why":"<=12 words why it fits"}, ... ${TOP_N} items]}`;
    ...
    // already-rejected codes, cap at TOP_N.
    const seen = new Set();
    const out = [];
    for (const r of ranked) {
      ...
      seen.add(occ.code);
      out.push({ code: occ.code, title: occ.title, definition: occ.definition, why: (r.why || '').trim() });
