...
// Lazy-loaded, cached corpus, indexed two ways:
//   byOcc  : Map<occupation, statements[]>           (LLM/title match)
//   byCode : Map<O*NET-SOC code, {occupation, statements[]}>  (direct lookup)
// Loaded once per process on first use. On failure we clear the promise so a
// later call retries.
...
function loadCorpus() {
  if (!corpusPromise) {
    corpusPromise = (async () => {
      ...
      const add = (occupation, code, statements) => {
        if (typeof occupation !== 'string' || !Array.isArray(statements)) return;
        const stmts = statements.filter(s => typeof s === 'string' && s.trim());
        byOcc.set(occupation, stmts);
        if (typeof code === 'string' && code.trim()) {
          byCode.set(code.trim(), { occupation, statements: stmts });
        }
      };
      // Accept [{occupation, code?, statements[]}], {occupation: statements[]},
      // or {occupation: {code?, statements[]}}.
      if (Array.isArray(data)) {
        for (const row of data) if (row) add(row.occupation, row.code, row.statements);
      } else if (data && typeof data === 'object') {
        for (const [occ, val] of Object.entries(data)) {
          if (Array.isArray(val)) add(occ, undefined, val);
          else if (val && typeof val === 'object') add(occ, val.code, val.statements);
        }
      }
      return { byOcc, byCode };
...
//
// Resolution order:
//   1. profile.occupationCode — exact O*NET-SOC lookup (no LLM call). Set when the
//      participant self-identified an occupation on the occupation-select screen.
//   2. profile.jobTitle — exact (case-insensitive) corpus occupation name (no LLM).
//   3. one-shot LLM occupation-match over the corpus list (the original path).
export async function retrieveTaskStatements(
  profile,
  { k = RETRIEVAL_K, topOccupations = RETRIEVAL_TOP_OCCUPATIONS } = {},
) {
  const { byOcc, byCode } = await loadCorpus();
  if (byOcc.size === 0) return { occupations: [], statements: [] };

  // 1) Direct code lookup — the most precise, and free.
  const code = (profile.occupationCode || '').trim();
  if (code && byCode.has(code)) {
    const { occupation, statements } = byCode.get(code);
    return { occupations: [occupation], statements: statements.slice(0, k) };
  }

  // 2) Exact occupation-name match — also free.
  const title = (profile.jobTitle || '').trim().toLowerCase();
  if (title) {
    for (const occ of byOcc.keys()) {
      if (occ.toLowerCase() === title) {
        return { occupations: [occ], statements: (byOcc.get(occ) || []).slice(0, k) };
      }
    }
  }

  // 3) Fall back to the one-shot LLM occupation-match over the corpus list.
  const occupations = [...byOcc.keys()];
